...
minsize = 10 # 포함시킬 최소 관광지 수 (첫 장소는 제외함)
...
class Route:
    def __init__(self, idlist=None):
        ...
        if comeback:
            routedistance += City(idlist[-1]).distance(City(0))
        ...
        self.price = sum(list(map(lambda x: City(x).price, idlist))) / self.size
    ...
    def fitness(self):
        fitness = ((self.star - 3) * self.review) / (self.distance * self.people * (self.price / 10000 + 1))
    ...
    def graph(self):
        ...
        if comeback:
            linelist.append([[City(0).x, City(0).y], routelist[-1]])
        for i in range(self.size-1):
